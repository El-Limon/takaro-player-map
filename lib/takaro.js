...
class TakaroClient {
  ...
  async getDeathEvents(gameServerId, startDate, endDate) {
    try {
      ...
      console.log('Death events API call:', JSON.stringify(body, null, 2));

      const response = await this.client.event.eventControllerSearch(body);

      // Filter by date range in JavaScript if dates provided
      ...
      if (error.response?.data?.meta?.error?.details) {
        console.error('Validation details:', error.response.data.meta.error.details);
      }
      throw new Error(message);
    }
  }
  ...
  // Search for players by item name
  async getPlayersByItem(gameServerId, itemName, startDate, endDate) {
    try {
      ...
      console.log('Calling Takaro item search API:', JSON.stringify(body, null, 2));
      ...
      const message = error.response?.data?.meta?.error?.message
        || error.response?.data?.message
        || error.message
        || 'Failed to search players by item';
      console.error('Takaro item search API error:', error.response?.data || error.message);
      throw new Error(message);
    }
  }
}
